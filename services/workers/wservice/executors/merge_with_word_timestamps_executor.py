...
class WServiceMergeWithWordTimestampsExecutor(BaseNodeExecutor):
    """
    WService 词级时间戳合并执行器。
    ...
    def execute_core_logic(self) -> Dict[str, Any]:
        ...
        logger.info(f"[{workflow_id}] 开始词级时间戳合并")
        ...
        if not validate_speaker_segments(speaker_segments):
            # 详细验证错误信息
            validation_error = self._get_validation_error(speaker_segments)
            logger.error(f"[{workflow_id}] 说话人时间段校验失败: {validation_error}")
        ...
        logger.info(f"[{workflow_id}] 词级合并完成，生成 {len(merged_segments)} 个片段")
    ...
    def _get_validation_error(self, speaker_segments: List[Dict]) -> str:
        """
        获取详细的验证错误信息。
    ...
    def get_required_output_fields(self) -> List[str]:
        ...
        词级合并的核心输出是 merged_segments_file。
        """

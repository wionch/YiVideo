...
    这里要强调: 整行文本移动了 必须保留[ID], 文本留空即可

```
重点讨论方案B, 前面尝试过纯文本对齐, 但是一直无法解决断句的问题. 同时LLM 大模型每次处理时存在动态波动, 因此其字幕处理的场景更加复杂, 更具随机性.

尝试过几个全文本方案但一直无法解决断句问题, 所以现在需要引入模型或者对齐工具链来实现
...
1. 每个node执行数据独立存储. 格式: `{任务id}:node:task_name`, 如: `video_to_subtitle_task:wservice:rebuild_subtitle_with_words`
...
1. {task\_id}应该是`"workflow_id`

2. node是`task_name`的`wservice`, 也就是第一个`.`之前的内容

3. {task\_name}是`task_name`的第一个`.`后面的内容

***

# 创建Qwen3-ASR节点

参考`faster_whisper.transcribe_audio`创建一个功能节点

...
-   本命令默认按此顺序使用 MCP（在不引入实现细节的前提下）：
-   1. `sequential-thinking`：对“分类法扫描结果”进行影响 × 不确定性排序，确保问题配额（≤5）优先覆盖高返工风险项。
-   2. `serena`：仅用于术语一致性与规范内部引用定位（例如找到同一概念在文档中的不同叫法），禁止读取实现细节来“替用户做技术决策”。
-   3. `context7`：当性能/合规/行业默认值缺失时，为“建议答案/推荐选项”提供可追溯的外部基准（并把来源简记进 `## Clarifications` 会话小节）。
-   若任一 MCP 不可用：必须在本次 Session 记录一条说明（缺失项、原因、替代来源、置信度影响）。
...
-   在顶部显著位置展示你的**推荐选项**，并附上清晰的理由（1-2 句话解释为什么这是最佳选择）。
-   格式为：`**推荐：** 选项 [X] - <理由>`

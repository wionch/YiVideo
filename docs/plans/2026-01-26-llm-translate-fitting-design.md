...
- **规范优先**：采用 Netflix Timed Text Style Guide 作为基线，默认 CPS=18（折中），CPL=42，单行输出（max_lines=1）。
...
- 角色：字幕翻译装词专家。
- 目标：语义忠实 + 时长/语速对齐。
- 规范：CPL 42，CPS 18，单行输出（max_lines=1）；**不得改变时间轴**。
...
- 若预算为 0，输出空文本并记录日志
- 不允许 `\\n` 换行符，保持单行文本
- 若原 segments 含 `words` 字段，翻译后清空 `words`，避免词级时间戳错配

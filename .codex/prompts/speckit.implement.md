...
1.5. **MCP 可用性预检（必需）**：
    ...
    - 确认 `sequential-thinking`、`serena`、`context7` 是否可用（至少能完成一次最小调用）。
    - 若有不可用项：必须在本次实施的状态报告中记录（缺失项、原因、替代信息源与风险），后续任务不得静默跳过 MCP 步骤。

2. **检查核查清单状态**（如果 FEATURE_DIR/checklists/ 存在）：

    - 扫描 checklists/ 目录中的所有核查清单文件
    - 对于每个核查清单，统计：
    ...
    - 计算总体状态：
        ...
        - **PASS**：所有核查清单都有 0 个未完成项目
        - **FAIL**：一个或多个核查清单有未完成项目

    - **如果有任何核查清单未完成**：

        - 显示带有未完成项目计数的表格
        - **停止**并（用中文）询问：“部分核查清单未完成。您是否仍要继续实施？(yes/no)”
        - 在继续之前等待用户响应
...
-   如果在预期时任何这些 MCP 服务不可用：
    ...
    -   任务不得静默跳过此 MCP 步骤。
-   **限制：避免工具诱导的超范围改动**：
    -   `serena` 用于“精确定位与邻域影响检查”，不得把一次任务扩大为大范围重构；若发现需要跨越任务边界的修改，必须回写 tasks.md 增加新任务并获得用户确认。

...
### `merge_for_tts` 节点输入说明

`faster_whisper.merge_for_tts` 节点在寻找输入文件时遵循以下逻辑：

1.  **优先使用 `subtitle_path` 参数**：如果在节点参数中明确提供了 `subtitle_path`，将直接使用该文件。此参数支持 `${{...}}` 动态传入。
2.  **自动查找 `generate_subtitle_files` 的输出**：如果未提供 `subtitle_path`，节点会自动寻找 `faster_whisper.generate_subtitle_files` 阶段的输出，并优先使用 `speaker_json_path`（带说话人信息的JSON文件）。
3.  **自动查找 `transcribe_audio` 的输出**：如果以上两项都未找到，节点会最后尝试使用 `faster_whisper.transcribe_audio` 阶段输出的 `segments_file`。

因此，虽然 `subtitle_path` 是可选的，但为了工作流的清晰和可控，推荐显式提供此参数。

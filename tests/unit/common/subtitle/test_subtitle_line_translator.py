...
def test_translate_lines_clears_words_for_translated_segments():
    segments = [
        {
            "start": 0.0,
            "end": 1.0,
    ...
    assert result["success"] is True
    assert result["translated_segments"][0]["words"] == []
    assert "words" not in result["translated_segments"][1]

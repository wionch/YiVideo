...
# Optimized to use Feature Name instead of Git Branch for documentation
...
# [OPTIMIZED] Use FEATURE_NAME (logical folder name) for documentation ID
# ensuring that records are correct even if user is on 'main' branch.
DOC_ID="$FEATURE_NAME"

NEW_PLAN="$IMPL_PLAN"
AGENT_TYPE="${1:-}"

# Agent-specific file paths (unchanged)
...
validate_environment() {
    # [OPTIMIZED] Check for feature directory instead of strict branch match
    ...
    if [[ ! -f "$NEW_PLAN" ]]; then
        ...
        log_info "Current context: $FEATURE_NAME"
        exit 1
    fi
...
#==============================================================================
# Plan Parsing Functions (Unchanged logic, just consolidated)
#==============================================================================
...
parse_plan_data() {
    ...
    log_info "Parsing plan data for feature: $DOC_ID"
...
create_new_agent_file() {
    ...
    # [OPTIMIZED] Use DOC_ID (feature name) instead of raw branch name
    ...
    if [[ -n "$escaped_lang" ]]; then
        ...
        recent_change="- $escaped_branch: Added new feature"
    fi
...
update_existing_agent_file() {
    ...
    # [OPTIMIZED] Use DOC_ID (feature name) for entries
    ...
    if [[ -n "$tech_stack" ]]; then
        new_change_entry="- $DOC_ID: Added $tech_stack"
    elif [[ -n "$NEW_DB" ]] && [[ "$NEW_DB" != "N/A" ]]; then
        new_change_entry="- $DOC_ID: Added $NEW_DB"
    fi
    
    # Process file line by line (Logic preserved from original, only variable changed above)
...
#==============================================================================
# Main Agent File Update Function (Simplified)
#==============================================================================
...
update_agent_file() {
    ...
    if [[ ! -f "$target_file" ]]; then
        ...
        if create_new_agent_file "$target_file" "$temp_file" "$project_name" "$current_date"; then
            mv "$temp_file" "$target_file"
            log_success "Created new $agent_name file"
        else
            rm -f "$temp_file"; return 1
        fi
    else
        update_existing_agent_file "$target_file" "$current_date" && log_success "Updated $agent_name file"
    fi
}

#==============================================================================
# Agent Selection (Simplified Case Statement)
#==============================================================================
...
main() {
    validate_environment
    log_info "=== Updating agent context for feature: $DOC_ID ===" # Log with Feature Name
    ...
    echo
    log_info "Summary:"
    [[ -n "$NEW_LANG" ]] && echo "  - Language: $NEW_LANG"

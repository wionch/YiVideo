...
-   [单任务/文件 HTTP API 参考](SINGLE_TASK_API_REFERENCE.md) - `/v1/tasks` 与 `/v1/files` 端点
...
-   `upload_audio_segments_to_minio` (bool, 节点可选): 是否上传音频片段目录到 MinIO，默认 false。**注意：此功能尚未实现，参数已预留。**
-   `delete_local_segments_after_upload` (bool, 节点可选): 上传成功后是否删除本地片段，默认 false。**注意：此功能尚未实现，参数已预留。**
-   `compress_segments_before_upload` (bool, 节点可选): 上传前是否压缩片段目录，默认 false。**注意：此功能尚未实现，参数已预留。**
-   `segment_compression_format` (string, 节点可选): 压缩格式，可选 "zip" 或 "tar.gz"，默认 "zip"。**注意：此功能尚未实现，参数已预留。**
-   `segment_compression_level` (string, 节点可选): 压缩级别，可选 "fast"、"default"、"best"，默认 "default"。**注意：此功能尚未实现，参数已预留。**
...
```json
{
    ...
    "all_audio_files": ["/share/workflows/{workflow_id}/audio/audio_separated/video_(Vocals)_UVR.flac", "/share/workflows/{workflow_id}/audio/audio_separated/video_(Instrumental)_UVR.flac"],
    "vocal_audio_minio_url": "http://host.docker.internal:9000/yivideo/{workflow_id}/audio/audio_separated/video_(Vocals)_UVR.flac",
    "all_audio_minio_urls": ["http://host.docker.internal:9000/yivideo/{workflow_id}/audio/audio_separated/video_(Vocals)_UVR.flac", "http://host.docker.internal:9000/yivideo/{workflow_id}/audio/audio_separated/video_(Instrumental)_UVR.flac"],
...
### 2. audio_separator.health_check

服务健康检查任务，用于监控 Audio Separator 服务的运行状态。

**功能描述**：检查 Audio Separator 服务的健康状态，包括模型加载状态、GPU 可用性等信息。主要用于系统监控和服务诊断。

**输入参数**：

-   无直接输入参数（此节点主要用于系统健康检查）。

**配置来源说明**：

-   无节点参数。健康状态信息从服务运行时环境获取。
...
```json
{
    "task_name": "audio_separator.health_check",
    "input_data": {}
}
```
...
```json
{
    ...
    "speaker_statistics": {
        "SPEAKER_00": { "segments": 80, "duration": 120.5, "words": 0 },
        "SPEAKER_01": { "segments": 68, "duration": 160.0, "words": 0 }
    },
    "total_speakers": 2,
    "total_segments": 148,
    "summary": "检测到 2 个说话人，共 148 个说话片段 (使用免费接口: pyannote/speaker-diarization-community-1)",
...
-   `diarization_file`: 说话人分离结果文件路径（JSON 格式）
-   `detected_speakers`: 检测到的说话人标签列表
-   `speaker_statistics`: 每个说话人的统计信息（片段数、总时长）
-   `total_speakers`: 检测到的说话人总数
-   `total_segments`: 总片段数
-   `summary`: 结果摘要文本
-   `execution_time`: 执行时间（秒）
-   `audio_source`: 使用的音频源
-   `api_type`: API 类型（free/paid）
-   `model_name`: 使用的模型名称
...
-   可选：`pyannote_audio.diarize_speakers`（如果未直接提供 `diarization_file` 参数）
...
-   保持与原始说话人分离结果的数据格式兼容
-   支持动态参数引用
-   支持从 MinIO URL 自动下载说话人分离文件

**单任务模式支持**：

**输入参数**:

-   `diarization_file` (string, 必需): 说话人分离结果文件路径，支持本地路径或 MinIO URL
-   `speaker` (string, 可选): 指定要获取的说话人标签，如 "SPEAKER_00"
...
-   `diarization_file`: 优先从 `input_data` 获取，回退到 `pyannote_audio.diarize_speakers` 阶段输出
-   `speaker`: 从 `input_data` 获取，可选参数
...
-   `diarization_file` 可以是本地路径或 MinIO URL
-   验证结果仅供参考，不影响原始分离结果
...
```json
{
    ...
    "json_path": "/share/workflows/{workflow_id}/subtitles/video_subtitle.json",
    "subtitle_files": {
        ...
        "json": "/share/workflows/{workflow_id}/subtitles/video_subtitle.json"
    }
}
```

> **注意**: `with_speakers` 和 `word_timestamps` 文件仅在有说话人信息或词级时间戳时生成。
...
1. **基础 SRT 文件** (`subtitle_path`): 标准字幕格式，始终生成
2. **完整 JSON 文件** (`json_path`): 包含所有元数据的结构化字幕数据，始终生成
3. **带说话人 SRT** (`subtitle_files.with_speakers`): 包含说话人信息的字幕（需要说话人数据）
4. **词级时间戳 JSON** (`subtitle_files.word_timestamps`): 精确的词级时间戳（需要启用词级时间戳）
